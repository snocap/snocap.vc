...
export function t(
  ...
  interpolations: StringInterpolations = {},
  options: I18nOptions = defaultOptions,
): string {
  const usingTranslations =
    translations[`./translations/${options.locale}.json`];
  const formatString =
    textToTranslate in usingTranslations
      ? usingTranslations[textToTranslate]
      : textToTranslate;
